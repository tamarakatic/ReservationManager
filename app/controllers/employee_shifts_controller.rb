class EmployeeShiftsController < ApplicationController
  ...
  def create
    ...
    @shift    = Shift.new(:work_day => Date.parse(params[:current_date]),
                          :start_at => Time.parse(params[:start_at]),
                          :end_at => Time.parse(params[:end_at]))

    @shift.save!
    @employee_shift = EmployeeShift.new(:employee_id => @employee.id,
                                        :shift_id => @shift.id,
                                        :seat_id => params[:role])
  ...
  def update
    ...
    @shift.update(:work_day => Date.parse(params[:current_date]),
                  :start_at => Time.parse(params[:start_at]),
                  :end_at => Time.parse(params[:end_at]))
